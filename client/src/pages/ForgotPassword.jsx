...
function ForgotPassword() {
  ...
  const onSubmit = async (values) => {
    try { await api.post('/auth/forgot-password', values); toast.success('Check your email for reset link'); }
  ...
  return (
    <AuthLayout title="Forgot password" subtitle="Enter your email to receive a reset link">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <button className={buttonClass} disabled={isSubmitting}>{isSubmitting ? 'Sending...' : 'Send reset link'}</button>
      </form>
