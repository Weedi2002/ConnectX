...
function Register() {
  ...
  const onSubmit = async (values) => {
    ...
    if (registerThunk.fulfilled.match(result)) { toast.success('Account created! Check your email to verify.'); navigate('/'); }
  ...
  return (
    <AuthLayout title="Create account" subtitle="Join ConnectX in seconds">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <input className={inputClass} placeholder="Username" {...register('username', { required: 'Username is required', minLength: { value: 3, message: 'Min 3 characters' } })} />
        ...
        <div>
          <input className={inputClass} type="password" placeholder="Password" {...register('password', { required: 'Password is required', minLength: { value: 8, message: 'Min 8 characters' } })} />
          {errors.password && <p className="mt-1 text-xs text-red-400">{errors.password.message}</p>}
        </div>
        <button className={buttonClass} disabled={isSubmitting}>{isSubmitting ? 'Creating...' : 'Create account'}</button>
      </form>
