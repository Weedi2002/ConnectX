    ...
    >
      ...
      >
        ...
        <textarea
          ...
          rows={2}
          className="glass-input mb-3 w-full resize-none px-4 py-2.5 text-sm"
        />
        ...
        {selected.length > 0 && (
          <div className="mb-3 flex flex-wrap gap-1.5">
            {selected.map((u) => (
              <span
                key={u._id}
                className="glass-card flex items-center gap-1.5 !rounded-full px-3 py-1 text-xs text-indigo-200"
              >
                {u.username}
                <button onClick={() => toggle(u)} className="hover:text-red-400">
                  ×
                </button>
              </span>
            ))}
        ...
        <input
          ...
          className="glass-input mb-2 w-full px-4 py-2.5 text-sm"
        />
            ...
            return (
              <li key={u._id}>
                <button
                  onClick={() => toggle(u)}
                  className={cn(
                    'flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left hover:bg-white/[0.06] transition-colors',
