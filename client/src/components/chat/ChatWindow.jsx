...
function ChatWindow() {
  ...
  const handleSmartReply = async () => {
    ...
    try {
      ...
      if (!data.suggestions?.length) toast('No suggestions');
    } catch (err) {
    ...
    <div className="relative flex h-full flex-col min-w-0">
      ...
      <header className="flex items-center gap-3 px-5 py-3 border-b border-white/[0.08]">
        <button
          ...
          className="rounded-lg p-1.5 text-slate-400 hover:bg-white/[0.08] md:hidden"
        >
          ←
        </button>
        <button
        ...
        <div className="ml-auto flex items-center gap-1">
          <button
            className="rounded-lg p-2.5 text-slate-400 hover:bg-white/[0.08] hover:text-slate-200"
            title="Voice call"
          >
            📞
          </button>
          ...
          <button
            ...
            disabled={aiBusy}
            className="rounded-lg p-2.5 text-slate-400 hover:bg-white/[0.08] hover:text-slate-200 disabled:opacity-50"
          >
            {aiBusy ? '⏳' : '✨'}
          </button>
        </div>
      ...
      {pinned.length > 0 && (
        <div className="glass flex items-center gap-2 mx-2 mb-1 rounded-lg px-3 py-1.5 text-[11px] text-slate-300">
          ...
          {pinned.length > 1 && (
            <span className="ml-auto text-slate-500">+{pinned.length - 1}</span>
          )}
        </div>
      )}
      ...
      {summary !== null && (
        ...
        >
          <div
            className="glass max-h-[80vh] w-full max-w-lg overflow-y-auto rounded-2xl p-5 shadow-glass"
