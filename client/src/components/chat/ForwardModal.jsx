...
function ForwardModal({ message, onClose }) {
  ...
  const forward = async (chatId) => {
    try { await api.post(`/messages/${message._id}/forward`, { chatId }); toast.success('Message forwarded'); onClose(); }
    catch (err) { toast.error(err.response?.data?.error || 'Failed to forward'); }
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass w-full max-w-sm rounded-3xl p-5 shadow-glass" onClick={(e) => e.stopPropagation()}>
        ...
        <ul className="max-h-72 space-y-1 overflow-y-auto">
          {chats.map((chat) => {
            ...
            return (
              <li key={chat._id}>
                <button onClick={() => forward(chat._id)} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left hover:bg-white/[0.06] transition-colors">
