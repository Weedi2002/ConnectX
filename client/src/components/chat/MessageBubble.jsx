...
function Attachment({ att }) {
  ...
  return (
    <a
      ...
      rel="noreferrer"
      download
      className="flex items-center gap-2 rounded-xl bg-white/[0.08] px-3 py-2 text-sm hover:bg-white/[0.12]"
    >
      <span>📄</span>
...
function ReplyPreview({ replyTo, own }) {
  ...
  return (
    <div
      className={cn(
        'mb-1.5 flex items-center gap-2 rounded-lg border-l-2 px-2 py-1 text-[11px]',
        own ? 'border-white/30 bg-white/[0.08]' : 'border-cyan-400/50 bg-white/[0.06]',
      )}
    >
    ...
    <div className="mt-1.5 flex flex-wrap gap-1">
      {Object.values(grouped).map((r) => (
        <button
          ...
          className="rounded-full bg-white/[0.1] px-2 py-0.5 text-[11px] hover:bg-white/[0.15]"
        >
          {r.emoji} {r.count}
        </button>
      ))}
...
function MessageBubble({ message, own, isGroup, chatId, onForward }) {
  ...
  return (
    <motion.div
      ...
      className={cn('group relative flex mb-3', own ? 'justify-end' : 'justify-start')}
    >
      <div
        className={cn(
          'max-w-[65%] rounded-2xl px-3.5 py-2.5 text-[12px] leading-relaxed',
          own ? 'bubble-own rounded-br-sm' : 'bubble-other rounded-bl-sm',
        )}
      >
        {isGroup && !own && (
          <p className="mb-1 text-[10px] font-semibold text-cyan-300">{message.sender?.username}</p>
        )}
        ...
        {translated !== null && (
          <div className="mt-1.5 rounded-lg bg-cyan-500/[0.1] border border-cyan-500/20 px-2 py-1 text-[11px] text-slate-300">
            🌐 {translated}
          </div>
        )}
        ...
        <div className="mt-1 flex items-center justify-end gap-1.5 text-[9px] opacity-50">
          ...
          {own && (
            <span className={message.status === 'read' ? 'text-cyan-300 opacity-100' : ''}>
          ...
          <button
            onClick={() => setMenu((m) => !m)}
            className="ml-0.5 hidden group-hover:inline hover:text-slate-200"
          >
            ⋯
          </button>
        </div>
        ...
        {menu && (
          <div
            className={cn(
              'absolute z-20 mt-1 w-36 glass rounded-xl p-1 shadow-glass text-[11px]',
              own ? 'right-0' : 'left-0',
            )}
          >
            <button
              onClick={handleReply}
              className="flex w-full items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-left hover:bg-white/[0.08]"
            >
              ↩ Reply
            </button>
            <button
            ...
            {own && (
              <button
                ...
                className="flex w-full items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-left text-red-400 hover:bg-white/[0.08]"
              >
                🗑 Delete
              </button>
            )}
            ...
            {message.content && (
              <button
                ...
                className="flex w-full items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-left hover:bg-white/[0.08]"
              >
                {translating ? '⏳…' : translated !== null ? '↺ Original' : '🌐 Translate'}
              </button>
            )}
          </div>
        )}
