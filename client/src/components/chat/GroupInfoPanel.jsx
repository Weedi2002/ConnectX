...
function GroupInfoPanel({ chat, onClose }) {
  ...
  return (
    <motion.aside
      ...
      className="flex h-full w-full flex-col border-l border-slate-800 bg-slate-900 md:w-80"
    >
          ...
          <div className="relative">
            ...
            {isAdmin && (
              <button
                ...
                className="absolute bottom-0 right-0 rounded-full bg-indigo-500 px-2 py-1 text-xs text-white"
              >
                ✎
              </button>
            )}
          ...
          {editing ? (
            <div className="mt-4 w-full space-y-2">
              <input
                ...
                className="w-full rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2 text-sm text-slate-100"
              />
              <textarea
                value={description}
              ...
              <button
                onClick={saveInfo}
                className="w-full rounded-lg bg-indigo-500 py-1.5 text-sm text-white"
              >
                Save
              </button>
            </div>
          ...
          {adding && (
            <div className="mb-3">
              <input
                ...
                className="mb-1 w-full rounded-lg border border-slate-700 bg-slate-800/60 px-3 py-2 text-sm text-slate-100"
              />
          ...
          <ul className="space-y-1">
            {chat.members.map((m) => {
              ...
              return (
                <li
                  key={m._id}
                  className="group flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-slate-800"
                >
                  ...
                  {isAdmin && m._id !== me._id && (
                    <div className="hidden items-center gap-1 group-hover:flex">
                      <button
                        ...
                        className="rounded px-1.5 py-0.5 text-[10px] text-slate-300 hover:bg-slate-700"
                      >
                        {memberIsAdmin ? 'Demote' : 'Promote'}
                      </button>
                      <button
                        onClick={() => removeMember(m._id)}
                        className="rounded px-1.5 py-0.5 text-[10px] text-red-400 hover:bg-slate-700"
                      >
                        Remove
                      </button>
                    </div>
      ...
      <footer className="space-y-2 border-t border-slate-800 p-4">
        <button
          onClick={toggleMute}
        ...
        <button
          onClick={leave}
          className={cn(
            'w-full rounded-lg bg-red-500/20 py-2 text-sm text-red-400 hover:bg-red-500/30',
          )}
        >
          Leave group
        </button>
