...
import {
  fetchIncoming, fetchSent, fetchFriends, fetchBlocked,
  acceptRequest, rejectRequest, cancelRequest, unblockUser,
...
export default function FriendRequestsPanel({ onClose }) {
  ...
  const accept = async (id) => { try { await dispatch(acceptRequest(id)); toast.success('Friend request accepted'); } catch { toast.error('Failed'); } };
  const reject = async (id) => { try { await dispatch(rejectRequest(id)); } catch { toast.error('Failed'); } };
  const cancel = async (id) => { try { await dispatch(cancelRequest(id)); } catch { toast.error('Failed'); } };
  const unblock = async (id) => { try { await dispatch(unblockUser(id)); toast.success('Unblocked'); } catch { toast.error('Failed'); } };
        ...
        <div className="max-h-[60vh] overflow-y-auto p-5 space-y-2">
          {tab === 'incoming' && (incoming.length === 0 ? (
            ...
            <div key={r._id} className="glass-card flex items-center gap-3 !rounded-xl p-3">
              ...
              <p className="min-w-0 flex-1 truncate text-sm text-slate-100">{r.sender?.username}</p>
              <button onClick={() => accept(r._id)} className="glass-btn-primary px-4 py-1.5 text-xs">Accept</button>
              <button onClick={() => reject(r._id)} className="glass-card !rounded-lg px-4 py-1.5 text-xs text-slate-300">Reject</button>
            </div>
          )))}
          ...
          {tab === 'sent' && (sent.length === 0 ? (
            ...
            <div key={r._id} className="glass-card flex items-center gap-3 !rounded-xl p-3">
              ...
              <button onClick={() => cancel(r._id)} className="glass-card !rounded-lg px-4 py-1.5 text-xs text-slate-300">Cancel</button>
            </div>
          )))}

          {tab === 'blocked' && (blocked.length === 0 ? (
            ...
            <div key={u._id} className="glass-card flex items-center gap-3 !rounded-xl p-3">
              ...
              <button onClick={() => unblock(u._id)} className="glass-card !rounded-lg px-4 py-1.5 text-xs text-slate-300">Unblock</button>
            </div>
          )))}
          ...
          {tab === 'friends' && (friends.length === 0 ? (
            ...
            <button key={u._id} onClick={() => openChatWith(u._id)} className="glass-card flex w-full items-center gap-3 !rounded-xl p-3 text-left">
