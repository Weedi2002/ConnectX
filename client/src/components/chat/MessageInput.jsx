    ...
    <div className="glass-input-bar border-t border-white/[0.06] px-4 py-3">
      ...
      {files.length > 0 && (
        <div className="mb-2 flex flex-wrap gap-1.5">
          {files.map((f, i) => (
            <span
              key={i}
              className="flex items-center gap-1 rounded-full bg-white/[0.08] px-2.5 py-1 text-[10px]"
            >
        ...
        <div className="mb-2.5 flex items-center gap-2 rounded-xl bg-red-500/10 border border-red-500/20 px-3 py-2 text-[12px]">
          ...
          <span className="ml-auto flex gap-1.5">
            <button
              ...
              className="rounded-lg px-2.5 py-1 text-[10px] text-red-300 hover:bg-white/[0.06]"
            >
              ✕
            </button>
            <button
              onPointerDown={stopRecording}
              className="glass-btn-primary px-2.5 py-1 text-[10px]"
            >
              ⏹
            </button>
          </span>
        ...
        <div className="mb-2 flex flex-wrap gap-1.5">
          {suggestions.map((s, i) => (
            <button
              ...
              className="rounded-full bg-cyan-500/10 border border-cyan-500/20 px-2.5 py-0.5 text-[10px] text-cyan-200 hover:bg-cyan-500/20"
            >
              {s}
            </button>
          ))}
      ...
      <form onSubmit={handleSend} className="flex items-center gap-2">
        <button
          ...
          className="flex-shrink-0 rounded-full p-2 text-slate-400 hover:bg-white/[0.08] hover:text-slate-200 transition-colors"
          title="Emoji"
        >
          😊
        </button>
        ...
        {recording ? (
          <span className="flex-shrink-0 rounded-full bg-red-500/20 px-3 py-2 text-[11px] text-red-300">
            ●
          </span>
        ) : (
          <button
            ...
            disabled={sending}
            className="flex-shrink-0 rounded-full p-2 text-slate-400 hover:bg-white/[0.08] hover:text-slate-200 disabled:opacity-50 transition-colors"
            title="Record"
          >
            🎤
          </button>
